...
namespace Rendering
{
	...
	{
		const float c3 = glm::cos(EULER_RADIANS.z);
		const float s3 = glm::sin(EULER_RADIANS.z);
		const float c2 = glm::cos(EULER_RADIANS.x);
		const float s2 = glm::sin(EULER_RADIANS.x);
		const float c1 = glm::cos(EULER_RADIANS.y);
		const float s1 = glm::sin(EULER_RADIANS.y);
